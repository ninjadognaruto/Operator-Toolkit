...
**A. Scan the workspace for context.** The user's question is often just the tip of the iceberg. Their workspace likely contains files that would dramatically improve the council's output. Before framing, quickly scan for relevant context:
...
- Any `knowledge/` folder (frameworks, domain expertise, past learnings)
- Any files the user explicitly referenced or attached
- Recent council transcripts (to avoid re-counciling the same ground)
- Any other context files relevant to the specific question (e.g., pricing question → look for revenue data, past launch results, audience research)

Use `Glob` and quick `Read` calls to find these. Don't spend more than 30 seconds. You're looking for the 2-3 files that give advisors enough context to give specific, grounded advice instead of generic takes.

**B. Frame the question.** Take the user's raw question AND the enriched context and reframe it as a clear, neutral prompt all five advisors will receive. The framed question should include:
...
Spawn all 5 advisors simultaneously as sub-agents. Each gets their advisor identity, the framed question, and a clear instruction to lean fully into their perspective without hedging. The synthesis comes later — each advisor's job is to represent their angle as strongly as possible.
...
This is what makes the council more than just "ask 5 times." It's the core of Karpathy's insight.
...
The chairman produces the final council output using this structure:
...
**File:** `council-report-[YYYY-MM-DD-HHMM].html`
...
3. **An agreement/disagreement visual** — a simple visual showing which advisors aligned and which diverged. Could be a grid, spectrum, or breakdown showing advisor positions. Keep it clean and scannable.
4. **Collapsible sections** for each advisor's full response (collapsed by default so the page isn't overwhelming, but available for digging in)
5. **Collapsible section** for the peer review highlights
6. **A footer** showing the timestamp and what was counciled

Styling: white background, subtle borders, readable sans-serif font (system font stack), soft accent colors to distinguish advisor sections. Professional briefing document feel. Nothing flashy.

Open/present the HTML file after generating it so the user can see it immediately.

### Step 6: Save the Full Transcript

Save the complete council transcript as `council-transcript-[YYYY-MM-DD-HHMM].md` in the same location. This includes:
...
The user sees the HTML report. The transcript is there for digging deeper or referencing specific advisor arguments later.
